...
@st.cache_data
def load_sva_familias_config(path: str = "sva_familias.yaml") -> Dict[str, Any]:
    """Taxonomia de famílias SVA (EBOOK / LOCAÇÃO / TV/STREAMING / OUTROS)."""
    try:
        with open(path, "r", encoding="utf-8") as f:
            return yaml.safe_load(f) or {}
    except FileNotFoundError:
...
@st.cache_data
def load_overrides_familia(path: str = "overrides_sva_familia.yaml") -> Dict[str, Any]:
    """Overrides manuais versionáveis (por descrição normalizada)."""
    try:
        with open(path, "r", encoding="utf-8") as f:
...
def extract_emitente_info(tree) -> Dict[str, str]:
    """Extrai CNPJ/CPF e nome do emitente (tolerante a namespaces)."""
    ns = get_ns(tree)
    root = tree.getroot()
    if ns:
        cnpj = root.xpath("string(.//n:emit/n:CNPJ)", namespaces=ns) or ""
        cpf = root.xpath("string(.//n:emit/n:CPF)", namespaces=ns) or ""
        xnome = root.xpath("string(.//n:emit/n:xNome)", namespaces=ns) or ""
    else:
        cnpj = root.xpath("string(.//emit/CNPJ)") or ""
        cpf = root.xpath("string(.//emit/CPF)") or ""
        xnome = root.xpath("string(.//emit/xNome)") or ""
    doc = (cnpj or cpf or "").strip()
    return {"emit_doc": doc, "emit_xNome": (xnome or "").strip()}


# ====================================================
# Classificação de FAMÍLIA SVA (para relatórios fiscais)
# ====================================================

FAMILIAS_SVA = ["SCM","SVA_EBOOK","SVA_LOCACAO","SVA_TV_STREAMING","SVA_OUTROS","INDEFINIDO"]

def _match_keywords(desc_norm: str, keywords: list[str]) -> bool:
    return any(k in desc_norm for k in (keywords or []))

def classify_sva_family(descricao: str, class_final_scm_sva: str, cfg: Dict[str, Any], overrides: Dict[str, Any]) -> Dict[str, Any]:
    desc_norm = normalize_text(descricao or "")
    key_norm = desc_norm.strip()

    if (class_final_scm_sva or "").upper() == "SCM":
        return {
            "familia_sugerida": "SCM",
            "familia_conf": 1.0,
            "familia_motivo": "Item classificado como SCM (regra SCM/SVA).",
            "familia_origem": "SCM_BASE",
            "familia_status": "AUTO",
            "key_norm": key_norm,
        }
    ...
    if ov and isinstance(ov, dict):
        fam = (ov.get("familia") or "").strip()
        if fam:
            return {
                "familia_sugerida": fam,
                "familia_conf": 0.99,
                "familia_motivo": "Override manual versionado para descrição normalizada.",
                "familia_origem": "OVERRIDE",
                "familia_status": "AUTO",
                "key_norm": key_norm,
            }

    familias = (cfg.get("familias") or {})
    thresholds = (cfg.get("thresholds") or {})
    auto_conf = float(thresholds.get("auto_conf", 0.90))
    review_conf = float(thresholds.get("review_conf", 0.70))
    prioridade = cfg.get("prioridade") or ["SVA_EBOOK","SVA_LOCACAO","SVA_TV_STREAMING","SVA_OUTROS"]

    melhor = {"familia": "INDEFINIDO", "conf": 0.0, "motivo": "Sem match", "origem": "REGRA"}

    for fam in prioridade:
        rule = familias.get(fam) or {}
        inc = rule.get("include") or []
        exc = rule.get("exclude") or []
        if inc and _match_keywords(desc_norm, inc) and (not exc or not _match_keywords(desc_norm, exc)):
            hits = sum(1 for k in inc if k in desc_norm)
            conf = min(0.60 + 0.10 * hits, 0.95)
            motivo = f"Match keywords ({hits}) para {fam}."
            if conf > melhor["conf"]:
                melhor = {"familia": fam, "conf": conf, "motivo": motivo, "origem": "REGRA"}

    if melhor["familia"] == "INDEFINIDO":
        melhor = {"familia": "SVA_OUTROS", "conf": 0.55, "motivo": "SVA sem match de família → OUTROS.", "origem": "FALLBACK"}

    status = "AUTO" if melhor["conf"] >= auto_conf else "REVISAR"
    if melhor["conf"] < review_conf:
        status = "REVISAR"

    return {
        "familia_sugerida": melhor["familia"],
        "familia_conf": float(melhor["conf"]),
        "familia_motivo": melhor["motivo"],
        "familia_origem": melhor["origem"],
        "familia_status": status,
        "key_norm": key_norm,
    }


def merge_overrides(overrides: Dict[str, Any], updates: Dict[str, str]) -> Dict[str, Any]:
    out = dict(overrides or {})
    for k, fam in (updates or {}).items():
        k = (k or "").strip()
        fam = (fam or "").strip()
        if not k or not fam:
            continue
...
def dump_overrides_yaml(overrides: Dict[str, Any]) -> str:
    lines = ["# overrides_sva_familia.yaml", "# Chave = descrição normalizada (sem acentos, minúscula).", ""]
    for k in sorted(overrides.keys()):
        v = overrides.get(k)
        fam = (v.get("familia") if isinstance(v, dict) else v) or ""
        fam = str(fam).strip()
        if fam:
            lines.append(f"\"{k}\":")
            lines.append(f'  familia: "{fam}"')
    lines.append("")
    return "\n".join(lines)


def gpt_classify_familias(descricoes: list[str], model: str) -> Dict[str, Dict[str, Any]]:
    try:
    ...
    client = OpenAI()
    allowed = ["SVA_EBOOK","SVA_LOCACAO","SVA_TV_STREAMING","SVA_OUTROS"]
    payload = {
        "results": [{"descricao": d} for d in descricoes],
        "familias_permitidas": allowed,
        "instrucoes": "Preencha cada item com familia (somente permitidas), conf (0-1) e motivo curto. Se dúvida: SVA_OUTROS e conf<=0.60. Responda APENAS JSON."
    }
    ...
    if isinstance(results, list):
        for r in results:
            ...
            if d and fam in allowed:
                out[d] = {"familia": fam, "conf": float(conf or 0.0), "motivo": motivo}
    return out
...
def generate_excel_report(
    ...
    df_familia_item: pd.DataFrame | None = None,
    df_reincidencia: pd.DataFrame | None = None,
    df_rev_familia: pd.DataFrame | None = None,
) -> io.BytesIO:
    """
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if df_familia is not None and not df_familia.empty:
            df_familia.to_excel(writer, sheet_name="Faturamento Familia", index=False)

        if df_familia_item is not None and not df_familia_item.empty:
            df_familia_item.to_excel(writer, sheet_name="Faturamento Fam Item", index=False)

        if df_reincidencia is not None and not df_reincidencia.empty:
            df_reincidencia.to_excel(writer, sheet_name="Reincidencia Clientes", index=False)

        if df_rev_familia is not None and not df_rev_familia.empty:
            df_rev_familia.to_excel(writer, sheet_name="Revisao Familias", index=False)
...
def main():
    ...
    # ============================
    # IA – Famílias SVA (Relatório)
    # ============================
    st.sidebar.subheader("IA – Famílias SVA")
    sva_cfg = load_sva_familias_config()
    overrides_fam = load_overrides_familia()

    usar_familias_sva = st.sidebar.checkbox("Ativar famílias SVA no relatório", value=True)
    limiar_gpt = float((sva_cfg.get("thresholds") or {}).get("gpt_conf", 0.65))

    usar_gpt_baixa_confianca = st.sidebar.checkbox("Usar GPT quando baixa confiança", value=False)
    gpt_model = st.sidebar.text_input("Modelo GPT (opcional)", value="gpt-4.1-mini")
